...
interface TabelPotonganCustomProps {
    data: PotonganCustomData[];
    listPegawai: any[];
    onDelete: (id: string) => void;
    onEdit?: (potongan: PotonganCustomData) => void;
}
...
export const TabelPotonganCustom = ({ data = [], listPegawai = [], onDelete, onEdit }: TabelPotonganCustomProps) => {
    ...
    const deductionsMap = useMemo(() => {
        const map: { [pegawaiIdOrName: string]: { [date: string]: PotonganCustomData[] } } = {};
        data.forEach(potongan => {
            const key = potongan.pegawai_id || potongan.nama_pegawai;
            if (!map[key]) map[key] = {};
            if (!map[key][potongan.tanggal_diberikan]) map[key][potongan.tanggal_diberikan] = [];
            map[key][potongan.tanggal_diberikan].push(potongan);
        });
        return map;
    }, [data]);
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div className="w-full min-h-[44px] flex flex-col gap-1.5 items-center justify-center relative">
                                                        {deductionsOnDay.length === 1 ? (
                                                            (() => {
                                                                const potongan = deductionsOnDay[0];
                                                                return (
                                                                    <div 
                                                                        key={potongan.id} 
                                                                        className="relative group/potongan bg-rose-50 hover:bg-rose-100 text-rose-800 border border-rose-200 rounded-lg p-1.5 shadow-2xs transition-all w-full flex flex-col gap-1 select-none"
                                                                    >
                                                                        {/* Main Nominal & Action Bar */}
                                                                        <div className="flex items-center justify-between gap-1 w-full">
                                                                            <span className="font-extrabold text-[11px] text-rose-700 tracking-tight">
                                                                                -Rp{potongan.nominal.toLocaleString('id-ID')}
                                                                            </span>
                                                                            
                                                                            <div className="flex items-center gap-1 opacity-90 group-hover/potongan:opacity-100 transition-opacity">
                                                                                {onEdit && (
                                                                                    <button
                                                                                        type="button"
                                                                                        onClick={(e) => {
                                                                                        ...
                                                                                        title={`Edit: ${potongan.keterangan || 'Tanpa keterangan'}`}
                                                                                    >
                                                                                        <Pencil size={11} />
                                                                                    </button>
                                                                                )}
                                                                        ...
                                                                        {/* Keterangan Tag */}
                                                                        {potongan.keterangan && (
                                                                            <div className="text-[9px] font-medium text-rose-600/90 truncate border-t border-rose-200/60 pt-0.5" title={potongan.keterangan}>
                                                                                {potongan.keterangan}
                                                                            </div>
                                                                        )}
                                                                        ...
                                                                        {potongan.keterangan && (
                                                                            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 hidden group-hover/potongan:flex flex-col bg-white border border-slate-200 text-slate-800 p-2.5 rounded-xl shadow-2xl w-48 z-[100] pointer-events-none transition-all">
                                                                                ...
                                                                                <div className="mt-1 font-bold text-rose-600 text-xs">
                                                                                    -Rp{potongan.nominal.toLocaleString('id-ID')}
                                                                                </div>
                                                            ...
                                                            (() => {
                                                                const totalNominal = deductionsOnDay.reduce((acc, p) => acc + (Number(p.nominal) || 0), 0);
                                                                return (
                                                                    <div className="relative group/cell bg-rose-50 hover:bg-rose-100 text-rose-800 border border-rose-300 rounded-lg p-1.5 shadow-2xs transition-all w-full flex flex-col gap-0.5 select-none cursor-pointer">
                                                                        <div className="flex items-center justify-between gap-1 w-full">
                                                                            <span className="font-extrabold text-[11px] text-rose-700 tracking-tight">
                                                                                -Rp{totalNominal.toLocaleString('id-ID')}
                                                                            </span>
                                                                            <span className="text-[9px] font-extrabold text-rose-700 bg-rose-200/80 px-1.5 py-0.2 rounded border border-rose-300/60">
                                                                                {deductionsOnDay.length} item
                                                                            </span>
                                                                        </div>
                                                                        <div className="text-[9px] font-medium text-rose-600/90 truncate border-t border-rose-200/60 pt-0.5">
                                                                            {deductionsOnDay.map(p => p.keterangan).filter(Boolean).join(', ')}
                                                                        </div>

                                                                        {/* Popover Breakdown on Hover */}
                                                                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 hidden group-hover/cell:flex flex-col bg-white border border-slate-200 text-slate-800 p-2.5 rounded-xl shadow-2xl w-56 z-[100] transition-all pointer-events-auto">
                                                                            ...
                                                                            <div className="flex flex-col gap-1.5 mt-2 max-h-40 overflow-y-auto scrollbar-thin">
                                                                                {deductionsOnDay.map((p) => (
                                                                                    <div key={p.id} className="flex items-center justify-between gap-2 p-1.5 bg-slate-50 rounded-lg border border-slate-200 text-xs">
                                                                                        <div className="flex flex-col min-w-0">
                                                                                            <span className="font-bold text-rose-700 text-[11px]">-Rp{p.nominal.toLocaleString('id-ID')}</span>
                                                                                            <span className="text-[10px] text-slate-600 truncate">{p.keterangan || "Tanpa keterangan"}</span>
                                                                                        </div>
