...
export function useMatrixJadwal() {
    ...
    const handleProsesTukarShift = async () => {
        if (!cellTujuan) return alert("Harap pilih jadwal tujuan terlebih dahulu.");
    ...
    const handleProsesGenerateMassal = async () => {
        if (!massalTanggalMulai || !massalTanggalSelesai) return alert("Harap lengkapi tanggal mulai, tanggal selesai, dan pilihan shift.");
        if (new Date(massalTanggalMulai) > new Date(massalTanggalSelesai)) return alert("Tanggal mulai tidak boleh lebih besar dari tanggal selesai!");
        if (selectedPegawaiIds.length === 0) return alert("Harap pilih minimal satu pegawai dari daftar target!");
    ...
    return {
        ...
        // State Data
        matrixKaryawan, isLoading, errorMsg, listPegawai, listMasterShifts,
